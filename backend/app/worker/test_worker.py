    ...
    async with async_session() as db:
        try:
            ...
            for i, reading_data in enumerate(readings):
                ...
                history = await history_crud.create(db, history_data)
            ...
            alert = await alert_crud.create(db, alert_data)
...
async def test_alert_evaluation():
    ...
    # Create test data
    device_id, alert_id = await create_test_data()
    ...
    async with async_session() as db:
        try:
            ...
            events = await alert_event_crud.get_by_alert(db, alert_id)
...
async def cleanup_test_data():
    ...
    async with async_session() as db:
        try:
            ...
            test_device = await device_crud.get_by_address(db, "test_address_001")
            if test_device:
                await device_crud.remove(db, test_device.id)
