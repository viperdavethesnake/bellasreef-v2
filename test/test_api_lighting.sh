...
echo "### 💡 Lighting Service - Full End-to-End Integration Test 💡 ###"
...
curl -s -X POST "${LIGHTING_URL}/lighting/runner/channels/${WHITES_CHANNEL_ID}/register?controller_address=64&channel_number=0" \
  -H "Authorization: Bearer $AUTH_TOKEN" | jq .
echo "Registering 'Blues' channel (HAL ID $BLUES_CHANNEL_ID) with lighting runner..."
curl -s -X POST "${LIGHTING_URL}/lighting/runner/channels/${BLUES_CHANNEL_ID}/register?controller_address=64&channel_number=1" \
  -H "Authorization: Bearer $AUTH_TOKEN" | jq .
echo "✅ All channels registered with lighting service."
...
ASSIGN_WHITES_RESPONSE=$(curl -s -X POST "${LIGHTING_URL}/lighting/assignments/assign-to-channel" \
  -H "Authorization: Bearer $AUTH_TOKEN" -H "Content-Type: application/json" \
  -d "{\"behavior_id\": ${WHITES_BEHAVIOR_ID}, \"channel_id\": ${WHITES_CHANNEL_ID}}")
ASSIGN_WHITES_ID=$(echo "$ASSIGN_WHITES_RESPONSE" | jq -r '.assignment.id')
echo "✅ Assigned behavior ${WHITES_BEHAVIOR_ID} to channel ${WHITES_CHANNEL_ID}."

echo "Assigning 'Royal Blues' to the 'Blues' channel..."
ASSIGN_BLUES_RESPONSE=$(curl -s -X POST "${LIGHTING_URL}/lighting/assignments/assign-to-channel" \
  -H "Authorization: Bearer $AUTH_TOKEN" -H "Content-Type: application/json" \
  -d "{\"behavior_id\": ${BLUES_BEHAVIOR_ID}, \"channel_id\": ${BLUES_CHANNEL_ID}}")
ASSIGN_BLUES_ID=$(echo "$ASSIGN_BLUES_RESPONSE" | jq -r '.assignment.id')
echo "✅ Assigned behavior ${BLUES_BEHAVIOR_ID} to channel ${BLUES_CHANNEL_ID}."
...
curl -s -X POST "${LIGHTING_URL}/lighting/scheduler/run-single-iteration" \
  -H "Authorization: Bearer $AUTH_TOKEN" | jq .
...
echo -e "\n🎉🎉🎉 LIGHTING SERVICE END-TO-END TEST PASSED! 🎉🎉🎉"
